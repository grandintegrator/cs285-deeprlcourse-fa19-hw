...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        if perform_random_action:
            action = self.env.env.env.action_space.sample()
        else:
            ...
            action = self.actor.get_action(obs=enc_last_obs)
            action = action[0]
        ...
        # TODO
        obs, reward, done, _ = self.env.step(action=action)

        # TODO store the result of taking this action into the replay buffer
        # HINT1: see replay buffer's store_effect function
        # HINT2: one of the arguments you'll need to pass in is self.replay_buffer_idx from above
        self.replay_buffer.store_effect(idx=self.replay_buffer_idx,
                                        action=action,
                                        reward=reward,
                                        done=done)
