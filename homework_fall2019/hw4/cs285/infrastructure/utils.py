...
def calculate_mean_prediction_error(env, action_sequence, models, data_statistics):
    ...
    for ac in action_sequence:
        ...
        ob = model.get_prediction(ob) # TODO(Q1) Get predicted next state using the model
    pred_states = np.squeeze(pred_states)

    # Calculate the mean prediction error here
    mpe = mean_squared_error(a=pred_states,
                             b=true_states) # TODO(Q1)
...
def sample_trajectories(env, policy,
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        path_dict = sample_trajectory(env=env,
                                      policy=policy,
                                      render=render,
                                      max_path_length=max_path_length,
                                      render_mode=render_mode)

        # Append the dictionary path into the full list of paths for this batch
        paths.append(path_dict)

        # How many time steps have been collected
        timesteps_this_batch += get_pathlength(path_dict)

    # Convert the list of dictionaries to array of trajectories
    # paths = convert_listofrollouts(paths=paths)
...
def sample_n_trajectories(env, policy, ntraj, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    for _ in range(ntraj):
        path_dict_single = sample_trajectory(env=env,
                                             policy=policy,
                                             max_path_length=max_path_length,
                                             render=render,
                                             render_mode=render_mode)
        paths.append(path_dict_single)
    # Convert the list of dictionaries to array of trajectories
    # paths = convert_listofrollouts(paths=paths)
