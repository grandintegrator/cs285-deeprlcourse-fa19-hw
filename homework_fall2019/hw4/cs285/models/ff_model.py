...
class FFModel(BaseModel):
    ...
    def __init__(self, sess, ac_dim, ob_dim, n_layers, size, learning_rate=0.001,
                 scope='dyn_model'):
    ...
    def define_forward_pass(self):
        ...
        obs_normalized = normalize(data=obs_unnormalized,
                                   mean=self.obs_mean_pl,
                                   std=self.obs_std_pl)
        # TODO(Q1) Define obs_normalized using obs_unnormalized,and self.obs_mean_pl and self.obs_std_pl
        acs_normalized = normalize(data=acs_unnormalized,
                                   mean=self.acs_mean_pl,
                                   std=self.acs_std_pl)
        ...
        self.delta_pred_unnormalized = unnormalize(data=self.delta_pred_normalized,
    ...
    def define_train_op(self):
        ...
        self.delta_labels_normalized = normalize(data=self.delta_labels,
                                                 mean=self.delta_mean_pl,
                                                 std=self.delta_std_pl) # TODO(Q1) Define a normalized version of delta_labels using self.delta_labels (which are unnormalized), and self.delta_mean_pl and self.delta_std_pl
        ...
        self.loss = tf.losses.mean_squared_error(labels=self.delta_labels_normalized,
                                                 predictions=self.delta_pred_normalized)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        if len(obs.shape)>1:
            ...
            actions = acs [None]
        # tf.squeeze()
        return self.sess.run([self.delta_pred_normalized],
