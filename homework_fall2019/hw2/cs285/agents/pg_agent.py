...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        indices = [index for index in range(0, len(rewards))]

        # 2) create a list where the entry at each index (t') is gamma^(t')
        discounts = [self.gamma**t_prime for t_prime in indices]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [discounts[index]*rewards[index] for index in indices]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            discounts = [self.gamma**(t_prime-start_time_index) for t_prime in indices]
            ...
            discounted_rtg = [discounts[index]*rewards[index] for index in indices]
