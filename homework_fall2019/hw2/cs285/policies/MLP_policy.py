...
class MLPPolicy(BasePolicy):
    ...
    def get_action(self, obs):
        if len(obs[0].shape) > 1:
            ...
            observation = obs[0][None]

        return self.sess.run(self.sample_ac, feed_dict={self.observations_pl: observation.reshape(1, self.ob_dim)})
...
class MLPPolicySL(MLPPolicy):
    ...
    def update(self, observations, actions, train_step):
        assert(self.training, 'Policy must be created with training=True in order to perform training updates...')
        _, mse_val = self.sess.run([self.train_op, self.loss],
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        w = tf.multiply(self.logprob_n, self.adv_n)

        self.loss = tf.reduce_sum(-1*w)
        ...
        if self.nn_baseline:
            ...
            self.baseline_loss = tf.losses.mean_squared_error(labels=self.targets_n,
    ...
    def run_baseline_prediction(self, obs):
        ...
        return self.sess.run(self.baseline_prediction, feed_dict={self.observations_pl: obs.reshape(len(obs), self.ob_dim)})

    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        assert(self.training, 'Policy must be created with training=True in order to perform training updates...')

        _, loss = self.sess.run([self.train_op, self.loss],
                                feed_dict={self.observations_pl: observations.reshape(len(observations), self.ob_dim),
